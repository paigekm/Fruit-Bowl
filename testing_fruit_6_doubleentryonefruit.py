...
def fill_fruitbowl(l):
    ...
    while cont == "y":
        ...
        result = test_fruit_present(l, type_of_fruit)
        if result == False:
            quantity_of_fruit = get_validated_integer("How many of that fruit would "
                                                  "you like to add?", 0, math.inf)
            print("You have entered {} {}s".format(quantity_of_fruit, type_of_fruit))
            confirm = input("press (A) to confirm, (B) to re-enter again or (C) to exit without changes: -> ")
...
def test_fruit_present(l, type_of_fruit):
    for i in range(0, len(l)):
        if type_of_fruit.lower() == l[i][0].lower():
            fruit_amount = get_validated_integer("Please enter how much fruit you want to add: ->",0,10)
            print("You have now added {} new {}s to your collection of {}s which were already in the bowl".format(fruit_amount, l[i][0], l[i][0]))
            print("." * 60)
            # access to original list, then just added one on
            l[i][1] += fruit_amount
            return True
    return False
